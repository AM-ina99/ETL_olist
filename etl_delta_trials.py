...
class Extract:  
    ...
    def extract_from_csvs(self, folder_path):
        ...
        #update actual_dfs
        products = actual_dfs['products']
        
        products['start_date'] = None
        products['end_date'] = None
        products['current_flag'] = None

        actual_dfs['products']= products
...
class CheckBeforeInsertion:
    ...
    def insert_scd_table(self, df, table_name):
        """Handle SCD tables (products/sellers) with versioning logic"""

        with self.engine.begin() as conn:  # Use transaction block
            table_pks = self.TPK.detect_primary_keys()
            existing_active = pd.read_sql(f"SELECT * FROM {table_name} WHERE current_flag = 'Y'", conn)
            if existing_active.empty:
                df['start_date'] = datetime.datetime.now()
                df['end_date'] = None
                df['current_flag'] = 'Y'
                df.to_sql(table_name, self.engine, index=False, if_exists='append')
                print(f"Initial SCD insert: {len(df)} rows")
                return
    ...
    def insert_regualr_tables(self, df, table_name): 
        table_pks = self.TPK.detect_primary_keys()
        try:    
            with self.engine.connect() as conn:
...
class Load:
    ...
    def load_clean_data(self,clean_dfs):
        first_load_order = ['sellers', 'customers', 'product_category_name_translation']            
        second_load_order = ['orders',  'order_payments', 'order_reviews', 'order_items', 'geolocation']
        # Load tables in dependency order
        print("\n Starting data load process...")
        for table_name in first_load_order:
            if table_name in clean_dfs:
                print(f"\nProcessing {table_name}...")
                self.CBI.insert_regualr_tables(clean_dfs[table_name], table_name)
            else:
                print(f"\n Skipping {table_name} - no data to load")

        #load scd table
        self.CBI.insert_scd_table(clean_dfs['products'],'products')

        #rest
        print("\n Starting data load process...")
        for table_name in second_load_order:
            if table_name in clean_dfs:
                print(f"\nProcessing {table_name}...")
                self.CBI.insert_regualr_tables(clean_dfs[table_name], table_name)
            else:
...
if __name__ == "__main__":
    ...
    log_paths = {
        ...
        "olist_ddl" : r'C:\Users\Menna Mohamed\Downloads\Atos\Final project\olist_ddl.txt',
